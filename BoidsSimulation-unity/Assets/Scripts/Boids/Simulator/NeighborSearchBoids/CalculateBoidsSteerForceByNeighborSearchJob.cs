...
{
    [BurstCompile]
    internal struct CalculateBoidsSteerForceByNeighborSearchJob : IJobParallelFor
    {
        ...
        public CalculateBoidsSteerForceByNeighborSearchJob(
        ...
        public void Execute(int ownIndex)
        {
            ...
            int minX = gridIndex.x - 1;
            int minY = gridIndex.y - 1;
            int minZ = gridIndex.z - 1;

            int maxX = gridIndex.x + 1;
            int maxY = gridIndex.y + 1;
            int maxZ = gridIndex.z + 1;
            ...
            for (int z = minZ; z <= maxZ; ++z)
            {
                ...
                if (!MathematicsUtility.IsCloseToGrid(ownPosition, key, _gridScale))
                {
                    continue;
                }
